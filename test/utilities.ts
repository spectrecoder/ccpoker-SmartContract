...
export function matrixMultiplication(
  ...
  assert(
    A.length === numRows * numCols,
    "Shape of A should be numRows x numCols"
  );
...
export function ecDecompress(
  ...
  assert(
    xArr.length === deltaArr.length,
    "Length of xArr should equal to the length of deltaArr"
  );
...
export function prepareDecryptData(
  ...
  const q =
    21888242871839275222246405745257275088548364400416034343698204186575808495617n;
  const delta0 = ecX2Delta(babyjub, BigInt(x0));
  const delta1 = ecX2Delta(babyjub, BigInt(x1));
  const flag0 = BigInt(num2Bits(BigInt(selector0), numCards)[cardIdx]);
  const flag1 = BigInt(num2Bits(BigInt(selector1), numCards)[cardIdx]);
...
export function prepareShuffleDeck(
  ...
  const deckX1: bigint[] = [];
  for (let i = 0; i < numCards; i++) {
    deckX0.push(BigInt(deck[0][i]));
  }
  for (let i = 0; i < numCards; i++) {
    deckX1.push(BigInt(deck[1][i]));
  }
  const deckDelta = recoverDeck(babyjub, deckX0, deckX1);
  return {
    X0: deckX0,
    X1: deckX1,
    Selector: [BigInt(deck[2][0]), BigInt(deck[3][0])],
